...
for run=1:size(SNR,2) 
 for i=1:trails
    bipolar=2*randi([0 1],1,length)-ones(1,length);%set bipolar signal
    rect_bipolar=rectpulse(bipolar,sampling_rate);%rectangular pulse shaping
    rrc_bipolar=rcosdesign(1,10,sampling_rate,'sqrt');%root raised cosine filter
    sig_rrc=upfirdn(bipolar,rrc_bipolar,sampling_rate);%shaped and oversampled RRC wave
    sig_rect=rect_bipolar;               %shaped and oversampled rect wave 
  
  Eb=sum(rrc_bipolar.^2)/2;  %Set the thershold
  ...
  matchfilter_rrc=conv(received_rrc,rrc_bipolar);
...
xlabel('frequency');
ylabel('PSD');
title('Bipolar RECT PSD');

%RRC plots
figure;
subplot(4,1,1);
t1=linspace(0,T_b*length,size(bipolar,2));
plot(t1,bipolar,'r+');
hold on;
